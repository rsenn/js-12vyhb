const puppeteer = require(/*"/home/roman/.nvm/versions/node/v12.13.1/lib/node_modules/puppeteer" */ "puppeteer");
const http = require("http");
const child_process = require("child_process");
const fs = require("fs");
const util = require("util"); //console.log("DOM: ", Object.keys(dom));
...
(async () => {
  ...
  \**************************************************************************/
    ...
    if(info && info.pages) {
      ...
      .then(async () => {
        ...
        div.container { display: flex; flex-flow: column nowrap; justify-content: flex-start; align-items: center; width: 100vw; }
        div.content { padding: 0; margin: 11mm 5mm 21mm 5mm; overflow: hidden; position: relative; min-width: 192mm; width: 192mm; min-height: 242mm;  height: 242mm; }
        ...
        </head><body><div class="container">${svgDivs.join("\n")}</div></body></html>`
        );
  ...
  function loadAndPush(file) {
    ...
    arr.push(output);
    /*  const openUrl = `file://${process.cwd()}/${file}`;
...
*/
  ...
  const testData = {
    ...
    receiver: new PostalAddress({
      lastName: "Free Software Foundation Europe e.V.",
 ...
 <script>${utilSrc}</script>
    ...
    await page.evaluate(c => {
      ...
      function printDigits(arr, position, parent, scale = 0.32) {
        ...
        //     position.y += printDigits.rect.height / scale;
